...
重新部署只构建所选 Hermes 客户端及其共享 `service` 镜像，不启动另一个 Hermes 客户端，也不自动拉起整套容器。
...
Hermes A/B 共用 Test 2 server memory，所以选择 A 或 B 都会清空同一份共享业务记忆；区别是脚本只停止所选 Hermes 客户端，不会同时操作或启动另一个客户端。用户、管理员、角色、授权、session 等 `rbac_*` 核心数据保持不变。

完整删除范围、local 目标和 `-NoCache` 用法见 [[配置文档-local.md#测试环境一键维护]]。
