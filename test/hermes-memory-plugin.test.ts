...
test("Team Memory ships as a real Hermes memory plugin", async () => {
  ...
  assert.match(plugin, /def _dev_tool_call_log_enabled/);
  assert.match(plugin, /def _log_tool_call/);
  assert.match(plugin, /def _read_tool_call_log/);
  assert.match(plugin, /TEAM_MEMORY_HERMES_TOOL_CALL_LOG/);
  assert.match(plugin, /team-memory-tool-calls\.jsonl/);
  assert.match(plugin, /"input": args/);
  assert.match(plugin, /entry\["output"\] = result/);
  assert.match(plugin, /"durationMs"/);
