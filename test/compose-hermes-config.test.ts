...
test("Hermes local compose setup and manual flow do not require server client tokens", async () => {
  ...
  assert.doesNotMatch(compose, /BOOTSTRAP_USER_PASSWORD/);
  ...
  assert.doesNotMatch(manualFlow, /\$env:BOOTSTRAP_USER_PASSWORD = "<test local admin password>"/);
  assert.match(manualFlow, /When prompted, enter a local root admin password/);
  assert.match(manualFlow, /Team Memory prompts for the user name and password/);
  assert.match(manualFlow, /returns `logged_in`/);
  ...
  assert.match(manualFlow, /team -- --config \/workspace\/config\/team-memory\.hermes-local\.json members create user:test1-readonly Test1ReadOnly role-researcher/);
  assert.match(manualFlow, /When prompted, enter the read-only user's password/);
  assert.match(manualFlow, /team -- logout/);
  assert.doesNotMatch(manualFlow, /run login user:test1-readonly/);
  assert.doesNotMatch(manualFlow, /TEST1_READONLY_PASSWORD/);
