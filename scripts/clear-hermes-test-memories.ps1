...
try {
  ...
  Invoke-Docker (@("compose") + $ComposeFiles + @("build", "service", "hermes-local", "memory-test-maintenance"))
  ...
  Invoke-Docker (@("compose") + $ComposeFiles + @(
    "run", "--rm", "--no-deps", "memory-test-maintenance", "node", "--experimental-strip-types",
    "/app/scripts/clear-test-memory.mjs", "--config", "/app/config/team-memory.service.json",
    "--skip-vectors", "--skip-filesystem-cas", "--object-store-cas-directory", "/test-object-store"
  ))

  Invoke-Docker (@("compose") + $ComposeFiles + @("up", "-d", "--force-recreate", "object-store", "service"))
  ...
  Write-Host "The dedicated test object-store CAS volume was cleared through the core maintenance module."
} finally {
  Pop-Location
}
