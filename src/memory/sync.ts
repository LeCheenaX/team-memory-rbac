...
import { InMemoryAuthorizedQuerySource } from "./retrieval.ts";
...
export interface LocalAuthorizedViewState {
  identity?: AuthorizedViewIdentity;
  snapshot?: MemoryActiveView;
  ...
  implements MemoryAdapter<AuthorizedSyncBatch, AuthorizedSyncRequest>
{
  ...
  async execute(
    ...
    const changeCommitIds = records.map((record) => record.commit.id);
    ...
    if (records.length === 0) {
      return { kind: "noop", identity, changeCommitIds: [] };
    }
    const delta = deltaFromRecords(records, active);
    if (delta === undefined) {
      return {
        kind: "replace",
    ...
    return { kind: "delta", identity, delta, changeCommitIds };
  }
}
...
export class InMemoryLocalAuthorizedViewStore
  implements LocalAuthorizedViewStore
{
  private state: LocalAuthorizedViewState = {
    pendingOperations: [],
    conflicts: [],
    valid: false,
  };

  inspect(): LocalAuthorizedViewState {
...
export interface LocalAuthorizedViewStore {
  inspect(): LocalAuthorizedViewState;
  replace(
    identity: AuthorizedViewIdentity,
    snapshot: MemoryActiveView,
...
export class AuthorizedViewSynchronizer {
